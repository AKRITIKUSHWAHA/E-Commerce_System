...
export default function ContactPage() {
  ...
  const [error,      setError]      = useState('');          // ✅ alert ki jagah inline error
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ 10 second timeout — zyada wait nahi hoga
    const timeout = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('Request timed out. Please try again.')), 10000)
    );

    try {
      await Promise.race([api.sendContactMessage(form), timeout]);
      setSent(true); // ✅ success hone par seedha dikhao
    } catch (err) {
  ...
  const handleCheckReply = async (e) => {
    ...
    try {
      const data = await Promise.race([api.getMyMessages(checkEmail), timeout]);
      setMyMessages(data.messages || []);
      setChecked(true);
    } catch (err) {
      setCheckError(err.message || 'Something went wrong');
    } finally {
      setChecking(false);
    }
  };
            ...
            <div className="check-reply-section">
              ...
              <form className="check-reply-form" onSubmit={handleCheckReply}>
                <input
                  type="email"
                  value={checkEmail}
                  onChange={e => { setCheckEmail(e.target.value); setChecked(false); setMyMessages([]); setCheckError(''); }}
              ...
              {/* ✅ Inline error – alert nahi */}
              {checkError && (
                <div className="cf-error">⚠️ {checkError}</div>
              )}

              {checked && myMessages.length === 0 && !checkError && (
                ...
                <div className="check-reply-list">
                  {myMessages.map(msg => (
                    <div key={msg.id} className={`check-reply-card ${msg.status}`}>
                      ...
                      {msg.subject && <div className="crc-subject">Subject: {msg.subject}</div>}
                      <div className="crc-message"><p>{msg.message}</p></div>
                      {msg.reply ? (
          ...
          {sent ? (
            ...
            <>
              ...
              <form className="contact-form" onSubmit={handleSubmit}>
                ...
                {/* ✅ Inline error message — alert popup nahi */}
                {error && (
                  <div className="cf-error">⚠️ {error}</div>
                )}
